...
import {
  obterTodasAsMesas,
  adicionarNovaMesa,
} from "./mesasService"; // Importar do novo serviço
...
export async function GET(request: NextRequest) {
  try {
    ...
    let mesasFiltradas = obterTodasAsMesas(); // Usar o serviço
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const mesasExistentes = obterTodasAsMesas(); // Usar o serviço
    ...
    // Adicionar a nova mesa usando o serviço
    ...
    return NextResponse.json(
      ...
      { status: 201 } // Retornar status 201 para criação bem-sucedida
    );
  } catch (error) {
