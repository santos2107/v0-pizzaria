...
export const obterTodasAsMesas = () => {
  return [...mesasDataLocal];
};

export const obterMesaPorId = (id: number) => {
  return mesasDataLocal.find((mesa) => mesa.id === id);
};
...
export const adicionarNovaMesa = (novaMesaData: Omit<Mesa, "id">) => {
  ...
  mesasDataLocal.push(novaMesa);
  return novaMesa;
};

export const atualizarMesaExistente = (id: number, dadosAtualizados: Partial<Omit<Mesa, "id">>) => {
  const index = mesasDataLocal.findIndex((mesa) => mesa.id === id);
  if (index !== -1) {
    mesasDataLocal[index] = { ...mesasDataLocal[index], ...dadosAtualizados };
    return mesasDataLocal[index];
  }
  return null;
};
...
export const deletarMesa = (id: number) => {
  const index = mesasDataLocal.findIndex((mesa) => mesa.id === id);
  if (index !== -1) {
    const mesaDeletada = mesasDataLocal.splice(index, 1)[0];
    return mesaDeletada;
  }
  return null;
};
...
export const juntarMesas = (idsMesasParaJuntar: number[], numeroMesaPrincipal: string) => {
  ...
  if (mesaPrincipal && numerosMesasJuntadas.length > 1) {
    return atualizarMesaExistente(mesaPrincipal.id, {
      ...
      observacoes: `${mesaPrincipal.observacoes || ""} (Combinada com: ${numerosMesasJuntadas.filter(n => n !== numeroMesaPrincipal).join(", ")})`.trim(),
    });
  }
...
export const separarMesas = (idMesaPrincipal: number) => {
  ...
  if (mesaPrincipal && mesaPrincipal.mesasCombinadas && mesaPrincipal.mesasCombinadas.length > 0) {
    ...
    mesaPrincipal.mesasCombinadas.forEach(numeroMesaCombinada => {
      const mesaCombinada = mesasDataLocal.find(m => m.numero === numeroMesaCombinada);
      if (mesaCombinada) {
        // Aqui, idealmente, você restauraria a capacidade original também
        atualizarMesaExistente(mesaCombinada.id, { status: "Disponível", mesaCombinadaPai: undefined, mesasCombinadas: [] });
      }
    });
    // Restaurar mesa principal
    // Idealmente, restaurar capacidade original
    return atualizarMesaExistente(idMesaPrincipal, { status: "Disponível", mesasCombinadas: [], observacoes: mesaPrincipal.observacoes?.replace(/\(Combinada com:.*?\)/, "").trim() });
  }
  return null;
};
